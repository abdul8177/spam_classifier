...
# Lint as: python3
"""BERT finetuning runner for GLUE tasks (including MNLI, SST2)."""
...
flags.DEFINE_string("predict_input_file", "test.tsv",
                    "Input file to be used in the prediction mode.")

flags.DEFINE_string("predict_output_file", None,
                    "Output file to be used in the prediction mode.")
...
tf.flags.DEFINE_string("exp_name", "experiment",
                       "[Optional] Name of the current experiment.")
...
class MnliProcessor(DataProcessor):
  ...
  def get_test_examples(self, input_file):
    """See base class."""
    return self._create_examples(self._read_tsv(input_file), "test")

  def get_custom_examples(self, input_file):
    """See base class."""
    return self._create_examples(self._read_tsv(input_file), "custom")
...
class MNLIProcessorDistillation(DataProcessor):
  """Processor for the MNLI data set (GLUE version) using distillation."""
...
class SST2ProcessorDistillation(DataProcessor):
  """Processor for the SST2 data set (GLUE version) using distillation."""
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      ...
      guid = "%s-%s" % (set_type, i)
      text_a = tokenization.convert_to_unicode(line[sentence_index])
      if label0_index and label1_index:
        label = [float(line[label0_index]), float(line[label1_index])]
      else:
        label = [1.0, 0]
      examples.append(
          InputExample(guid=guid, text_a=text_a, text_b=None, label=label))
